public class SecurityPassMaker
{
    ...
    {
        if (support is Security && IsSecurityPriorityPersonnel((Security)support))
        {
            return $"{support.Title} Priority Personnel";
        }
        return support is Staff ? support.Title : "Too Important for a Security Pass";
    }

    public bool IsSecurityPriorityPersonnel(Security person)
    {
        return person.GetType() != typeof(SecurityJunior) 
            && person.GetType() != typeof(SecurityIntern)
            && person.GetType() != typeof(PoliceLiaison);
    }
}
